...
// --- RANDOM FOREST MODEL ---
const RF = {
    // Feature importances from your trained RF model
    feature_importance: {
        Fazekas: 0.176485,
        age: 0.145995,
        PS: 0.101065,
    ...
    // Baseline probability (mean dementia rate in training data)
    baseline_prob: 0.25,
    auc: 0.864,
    sensitivity: 0.938, // tuned threshold
};
...
function computeRF(inputs) {
    // Simplified RF prediction using feature importances
    // This creates a weighted contribution based on how far each feature is from baseline
    const contributions = {};
    let totalContribution = 0;

    for (const [feat, importance] of Object.entries(RF.feature_importance)) {
        const val = inputs[feat] ?? 0;
        const z = standardize(feat, val);
        // Contribution is based on standardized value * importance
        // Positive z = higher risk (for most features), negative z = lower risk
        const contrib = z * importance;
        contributions[feat] = contrib;
        totalContribution += contrib;
    }

    // Scale the total contribution to probability space
    // This is a simplified model - actual RF uses decision trees
    const scaledContribution = totalContribution * 0.5; // scaling factor
    const prob = Math.max(0, Math.min(1, RF.baseline_prob + scaledContribution));
...
export default function App() {
    ...
    const [selectedModel, setSelectedModel] = useState("LR"); // "LR" or "RF"

    const handleChange = (feat, val) =>
        setInputs((prev) => ({ ...prev, [feat]: val }));

    const lrResult = computeLR(inputs);
    const rfResult = computeRF(inputs);

    const result = selectedModel === "LR" ? lrResult : rfResult;
    const modelInfo = selectedModel === "LR" ? LR : RF;

    const sortedContribs = Object.entries(result.contributions).sort(
          ...
          `,
            ...
            >
                ...
                <div style={{ maxWidth: 1160, margin: "0 auto", padding: "32px 24px" }}>
                    ...
                    >
                        {/* LEFT: Input Form (same as before) */}
                        ...
                        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
                                    ...
                                    <span style={{ display: "flex", gap: 12 }}>
                                        <span>
                                            ...
                                            <span style={{ color: "#38BDF8" }}>
                                                {modelInfo.auc.toFixed(3)}
                                            </span>
                                        </span>
                                        <span>
                                            Sens{" "}
                                            <span style={{ color: "#38BDF8" }}>
                                                {modelInfo.sensitivity.toFixed(3)}
                                            </span>
                                        </span>
                                    </span>
                                </div>
                            ...
                            {showXAI && (
                                ...
                                >
                                    ...
                                    >
                                        {selectedModel === "LR"
                                            ? "XAI method: Logistic Regression coefficient × standardised feature value. Positive values push toward dementia; negative values are protective."
                                            : "XAI method: Random Forest feature importance × standardised feature value. Shows relative contribution of each feature to the prediction."}
                                    </div>
                                </div>
                            )}
